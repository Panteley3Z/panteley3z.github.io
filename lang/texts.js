const ruGameDescription = "Простая игра, где с помощью кольца нужно ловить разноцветные шарики. В самом начале кольцо имеет размеры самого маленького шарика, поэтому надо искать и ловить соответствующий шарик. Когда шарик пойман, кольцо окрашивается в цвет пойманного шарика и увеличивается до размера следующего. Игра закончится когда все шарики будут пойманы."
const enGameDescription = "Simple game where you use a ring to catch multicolored balls. At the very beginning, the ring is the size of the smallest ball, so you have to look for and catch the corresponding ball. When the ball is caught, the ring turns into the color of the caught ball and increases to the size of the next one. The game will end when all the balls are caught."

const ruWinString = "вы поймали все шарики! 🥳";
const enWinString = "you caught all the bubbles! 🥳";

const ruCalendarDescription = "Отображает текущий месяц года с возможностью перехода к предыдущим или последующим месяцам.";
const enCalendarDescription = "Displays the current month of the year with the ability to navigate to previous or next months.";

const ruClockDescription = "Часы в виде классического электронного циферблата. Созданы на основе &lt;table&gt;";
const enClockDescription = "A clock in the form of a classic electronic dial. Created based on &lt;table&gt;";

const ruColorPickerDescription = "Небольшой инструмент для подбора палитры цветов. Всего 5 цветов, которые при каждом нажатии на кнопку обновления принимают случайные значения. Можно зафиксировать понравившийся цвет, нажав на значок замка, который находится на выбранном цвете. В дополнительном поле отображаются выбранные цвета в формате rgb().";
const enColorPickerDescription = "A small tool for selecting a color palette. There are 5 colors, which take random values with each click on the update button. You can lock your favorite color by clicking on the lock icon located on the selected color. The additional field displays the selected colors in RGB format.";

const ruSummaryPhrase = "Если Вас заинтересовала моя кандидатура или есть какие-либо предложения - свяжитесь со мной любым удобным для Вас способом.";
const enSummaryPhrase = "If you are interested in my CV or have any offers, please contact me in a way that suits you best.";

const ruThanksFeedback = "Спасибо за обратную связь!";
const enThanksFeedback = "Thank you for your feedback!";

const ruWillWait = "Буду ждать с нетерпением! <span class='hello-hand animate'>🎩</span>";
const enWillWait = "I will be looking forward to it! <span class='hello-hand animate'>🎩</span>";

const ruAutobio1 = "Мне <span class='age-count'>35</span> лет, за спиной высшее образование и работа, не связанные с программированием.";
const enAutobio1 = "I am <span class='age-count'>35</span> years old, with a university degree and a job behind my back, not related to programming.";

const ruAutobio2 = "Однако мне всегда было очень интересно это направление, и теперь я посвящаю большую часть свободного времени изучению frontend-разработки.";
const enAutobio2 = "However, I have always been very interested in this field, and now I devote a large part of my free time to learning frontend development.";

const ruWarnMessage = "Внимание! Сайт находится на стадии разработки.";
const enWarnMessage = "Attention! The site is under development.";

const ruWhatIUse = "Кстати, при написании этого сайта я использовал только HTML, CSS и JavaScript.";
const enWhatIUse = "By the way, when writing this site, I used only HTML, CSS and JavaScript.";

const ruIntroDescForProjects = "Ниже представлены мои мини-проекты с краткими описаниями к ним. Список будет пополняться.";
const enIntroDescForProjects = "Below are my mini-pet-projects with brief descriptions of them. The list will be updated.";

export const TEXTS = {
    RU_game: ruGameDescription,
    EN_game: enGameDescription,
    RU_gameWin: ruWinString,
    EN_gameWin: enWinString,
    RU_calendar: ruCalendarDescription,
    EN_calendar: enCalendarDescription,
    RU_clock: ruClockDescription,
    EN_clock: enClockDescription,
    RU_ColorPicker: ruColorPickerDescription,
    EN_ColorPicker: enColorPickerDescription,
    RU_SummaryPhrase: ruSummaryPhrase,
    EN_SummaryPhrase: enSummaryPhrase,
    RU_ThanksFeedback: ruThanksFeedback,
    EN_ThanksFeedback: enThanksFeedback,
    RU_WillWait: ruWillWait,
    EN_WillWait: enWillWait,
    RU_Autobio1: ruAutobio1,
    EN_Autobio1: enAutobio1,
    RU_Autobio2: ruAutobio2,
    EN_Autobio2: enAutobio2,
    RU_WarnMessage: ruWarnMessage,
    EN_WarnMessage: enWarnMessage,
    RU_WhatIUse: ruWhatIUse,
    EN_WhatIUse: enWhatIUse,
    RU_IntroDescForProjects: ruIntroDescForProjects,
    EN_IntroDescForProjects: enIntroDescForProjects
}